...
def merge_args_with_config(args) -> SimpleNamespace:
    ...
    label_mode = get('labels.mode', 'other')
    ...
    return SimpleNamespace(
        ...
        ts_ma_window=ts_ma_window, img_w=img_w, img_h=img_h, img_dpi=img_dpi, label_mode=label_mode,
...
def build_dataset(csv_path: Path,
    ...
    for h in horizon_list:
        if h == 0 or label_mode == 'in_sample':
            lab = make_labels_in_sample(vol_src, median_window=image_windows[0], drop_na=True)
            print('## [LABEL GENERATION IN SAMPLE] ##')
        elif label_mode == 'overnight':                
            #
            lab = make_labels_overnight(
                ohlcv, median_window=median_window,
                tz_local="America/New_York", open_time="09:30", close_time="16:00",
                bar_label="end"
            )
        elif label_mode == 'ternary':
            lab = make_labels_ternary(vol_src, horizon_days=h, median_window=median_window, drop_na=True)
        else:
